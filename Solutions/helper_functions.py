...
def create_equatorial_distance_feature(df) -> pd.DataFrame:
    ...
    num_bins = 10
    df["distance_from_equator_bin"] = pd.cut(
        df["distance_from_equator"], bins=num_bins, labels=False
    )
...
def scale_features(X_train, X_test, feature_cols) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    rs = StandardScaler().fit(X_train[feature_cols])
...
def transform_target(
    ...
    if (y_train > 0).all() and (y_test > 0).all():
        transformer = PowerTransformer(method="box-cox").fit(
            y_train.values.reshape(-1, 1)
        )
    else:
        transformer = PowerTransformer(method="yeo-johnson").fit(
            y_train.values.reshape(-1, 1)
        )
...
def backward_stepwise_train(X_train, y_train, feature_cols):
    ...
    xtrain, xval, ytrain, yval = train_test_split(
        X_train[feature_cols], y_train, test_size=0.2, random_state=1
    )
    ...
    rmse = np.sqrt(
        mean_squared_error(
            target_transformer.inverse_transform(yval),
            target_transformer.inverse_transform(model.predict(xval)),
        )
    )
    ...
    for i in range(1, k):
        ...
        for feat in remaining_features:
            ...
            rmse = np.sqrt(
                mean_squared_error(
                    target_transformer.inverse_transform(yval),
                    target_transformer.inverse_transform(model.predict(xval[features])),
                )
            )

            # r2 = model.score(xval[features], yval)
            # print(features, f"  RMSE: {rmse}  ", f"R2: {r2}\n")
        ...
        # Save the best_rmse in the rmse_list
